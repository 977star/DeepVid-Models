# Third-party notices

## SenseVoiceSmall

- Upstream project: [QwenAudio/SenseVoice](https://github.com/QwenAudio/SenseVoice)
- Model package used here: `sherpa-onnx-sense-voice-zh-en-ja-ko-yue-int8-2024-07-17`
- Distribution source: [k2-fsa/sherpa-onnx model releases](https://github.com/k2-fsa/sherpa-onnx/releases/tag/asr-models)
- Model licence: [FunASR Model Open Source License Agreement](https://github.com/modelscope/FunASR/blob/main/LICENSE.model.md)

The official SenseVoice project states that commercial use is permitted subject to the model licence. Anyone redistributing or using this model must comply with that licence and retain applicable attribution and notices.

## Silero VAD

- Upstream project: [snakers4/silero-vad](https://github.com/snakers4/silero-vad)
- Distribution source: [k2-fsa/sherpa-onnx model releases](https://github.com/k2-fsa/sherpa-onnx/releases/tag/asr-models)

The VAD file is retained with its upstream provenance. Its applicable upstream licence and notices must be preserved by downstream users.

## Planned client runtime (not distributed by this repository)

DeepVid Studio will use [sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx), licensed under Apache-2.0, to load the model locally. The runtime is delivered with the application and is not a model-release asset in this repository.
